...
type RestOf<T extends IShipment | NewShipment> = Omit<T, 'createdAt'> & {
...
export class ShipmentService {
  ...
  protected convertDateFromClient<T extends IShipment | NewShipment | PartialUpdateShipment>(shipment: T): RestOf<T> {
    return {
      ...shipment,
      createdAt: shipment.createdAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restShipment: RestShipment): IShipment {
    return {
      ...restShipment,
      createdAt: restShipment.createdAt ? dayjs(restShipment.createdAt) : undefined,
    };
  }
